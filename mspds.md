...
This document proposed a documentation system for software projects, especially suited for student and small team work (or solo developers + AI agents).

We follow a few principles:

1. **Clarity**: Documentation should be clear and concise, avoiding unnecessary jargon.
2. **Accessibility**: Information should be easy to find and navigate, with a logical structure.
3. **Relevance**: Include only information that is necessary for the intended audience and purpose.
4. **Hard boundaries**: Do not repeat information across different documents. Instead, link to the relevant document or section.
5. **Single source of truth**: Each piece of information should have exactly one location in the documentation system.
6. **Up-to-date**: Keep documentation current with the latest project changes and developments.
7. **AI-friendly**: Consider the needs of AI agents that may interact with the documentation. Keep information structured and explicit. Keep each file focused and short (ideally under 200 lines).
...
- **guidelines/**: Contains process and “how to” documents for developers. Examples: `dev-environment.md`, `coding-standards.md`, `contribution-process.md`, `git-workflow.md`, `code-review.md`.
- **knowledge/**: Contains technology and “how it works” documents. Examples: `websockets.md`, `redux.md`, `authentication.md`, `deployment-pipeline.md`. Use this for tutorials, explanations, or deep dives into technologies, libraries, or patterns used in the project.
...
When documenting requirements, use user stories to describe desired functionality from the perspective of stakeholders. For each user story, include clear acceptance criteria that define when the story is considered complete.

In the design document, cover key technical details such as data models, API endpoints, and testing strategies. Be explicit and structured to help both humans and AI agents understand the design.

For the tasks document, break down the work into clear, actionable tasks and subtasks. Write each task so that an AI agent can easily interpret and assist with it. Keep tasks focused and concise, and reference the relevant requirements to provide context.
...
- **User Guide**: Detailed instructions on how to use the main features of the application, with screenshots or examples where helpful. If the application has a public API, include API documentation or links to it.
  - If the application is a service that integrates with other tools, provide integration guides. In this case, consider including an `llm.txt` file to serve as a guideline for AI agents.
