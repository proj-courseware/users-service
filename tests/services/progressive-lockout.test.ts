...
import { AuthenticationService, DEFAULT_AUTH_CONFIG } from "@/services/authentication.service";
...
describe("Progressive Account Lockout", () => {
  ...
  describe("Progressive Lockout Calculation", () => {
    it("should calculate progressive lockout durations correctly", async () => {
      ...
      const firstDuration = (authService as any).calculateProgressiveLockoutDuration(3, testConfig);
      expect(firstDuration).toBe(5 * 60 * 1000); // 5 minutes in ms

      // Second lockout (4th attempt): 2 * 5 minutes = 10 minutes  
      const secondDuration = (authService as any).calculateProgressiveLockoutDuration(4, testConfig);
      expect(secondDuration).toBe(10 * 60 * 1000); // 10 minutes in ms

      // Third lockout (5th attempt): 4 * 5 minutes = 20 minutes
      const thirdDuration = (authService as any).calculateProgressiveLockoutDuration(5, testConfig);
      expect(thirdDuration).toBe(20 * 60 * 1000); // 20 minutes in ms

      // Fourth lockout (6th attempt): 8 * 5 minutes = 40 minutes
      const fourthDuration = (authService as any).calculateProgressiveLockoutDuration(6, testConfig);
      expect(fourthDuration).toBe(40 * 60 * 1000); // 40 minutes in ms

      // Should cap at maximum duration (2 hours)
      const maxDuration = (authService as any).calculateProgressiveLockoutDuration(20, testConfig);
  ...
  describe("Failed Login Attempts", () => {
    it("should track failed attempts without locking initially", async () => {
      ...
      await expect(authService.loginWithPassword(loginCredentials, testConfig))
    ...
    it("should lock account after max failed attempts with progressive duration", async () => {
      ...
      const lockDuration = user!.accountLockedUntil!.getTime() - user!.accountLockedAt!.getTime();
    ...
    it("should provide informative lockout message with unlock time", async () => {
      ...
      try {
        ...
        expect((error as AccountLockedError).message).toContain("Account is locked");
        expect((error as AccountLockedError).message).toContain("Account will be unlocked at");
      }
    });
  });
  ...
  describe("Progressive Lockout Behavior", () => {
    it("should increase lockout duration with repeated violations", async () => {
      ...
      let user = await userRepository.findByEmail(testUser.primaryEmail);
      const firstLockDuration = user!.accountLockedUntil!.getTime() - user!.accountLockedAt!.getTime();
      ...
      const secondLockDuration = user!.accountLockedUntil!.getTime() - user!.accountLockedAt!.getTime();
    ...
    it("should cap lockout duration at maximum configured time", async () => {
      ...
      await userRepository.updateLoginAttempts(testUser.primaryEmail, 19, false);
  ...
  describe("Time-Based Lockout Expiry", () => {
    it("should automatically unlock account when lockout expires", async () => {
      ...
      await userRepository.updateLoginAttempts(testUser.primaryEmail, 3, true, expiredTime);
    ...
    it("should correctly identify when account is currently locked", async () => {
      ...
      // Should be locked
      let isLocked = await userRepository.isAccountCurrentlyLocked(testUser.primaryEmail);
      ...
      isLocked = await userRepository.isAccountCurrentlyLocked(testUser.primaryEmail);
  ...
  describe("Standard vs Progressive Lockout", () => {
    it("should use standard lockout when progressive lockout is disabled", async () => {
      ...
      for (let i = 0; i < testConfig.maxFailedAttempts; i++) {
        await expect(authService.loginWithPassword(loginCredentials, standardConfig))
          .rejects.toThrow(InvalidCredentialsError);
      }
      ...
      const lockDuration2 = user2!.accountLockedUntil!.getTime() - user2!.accountLockedAt!.getTime();
