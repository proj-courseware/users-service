...
describe("CSRFService", () => {
  ...
  describe("validateToken", () => {
    ...
    it("should handle malformed hash gracefully", () => {
      ...
      const isValid1 = csrfService.validateToken(
        tokenData.token,
        "malformed-hash",
      );
      ...
      const isValid2 = csrfService.validateToken(
        tokenData.token,
        "hash:not-a-number",
      );
      ...
      const isValid3 = csrfService.validateToken(
        tokenData.token,
        "hash:123:extra",
      );
  ...
  describe("timing attack protection", () => {
    it("should take similar time for valid and invalid tokens", async () => {
      ...
      csrfService.validateToken(
        "invalid-token-same-length-as-real",
        tokenData.hash,
      );
  ...
  describe("configuration", () => {
    it("should use custom configuration", () => {
      ...
      const isValid = customService.validateToken(
        tokenData.token,
        tokenData.hash,
      );
      expect(isValid).toBe(true);
    ...
    it("should reject tokens from different secrets", () => {
      const service1 = new CSRFService({
        secret: "secret1-32-characters-long-string",
      });
      const service2 = new CSRFService({
        secret: "secret2-32-characters-long-string",
      });
